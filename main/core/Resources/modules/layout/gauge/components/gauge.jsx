...
class GaugeText extends Component {
  ...
  updateText(currentValue, newValue) {
    const writeValue = (value) => this.props.displayValue(value) + (this.props.unit || '')
  ...
  render() {
    const textStartValue = !this.props.preFilled ? 0 : this.props.value
    ...
    return (
      <text
        ...
        fontSize={textPixels}
        textAnchor="middle"
        transform={`translate(${this.props.radius}, ${textRiseScaleY(constants.GAUGE_TEXT_POSITION)})`}
      >
        {this.props.displayValue(textStartValue) + (this.props.unit || '')}
      </text>
    )
  }
}
