...
const reducer = combineReducers({
  ltiResource: makeReducer({}, {
    [makeInstanceAction(RESOURCE_LOAD, selectors.STORE_NAME)]: (state, action) => action.resourceData.ltiResource || state,
    [FORM_SUBMIT_SUCCESS+'/'+selectors.STORE_NAME+'.ltiResourceForm']: (state, action) => action.updatedData
  }),
  ltiApps: makeReducer([], {
    [makeInstanceAction(RESOURCE_LOAD, selectors.STORE_NAME)]: (state, action) => action.resourceData.ltiApps || state
  }),
