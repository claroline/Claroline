...
function choiceCSSEdit(nbResponses) {   
    $("#ujm_exobundle_interactionqcmtype_choices").children("div").each(function(index) {
            $("#newTable").append('<tr class="ligne_choice" >  </tr>');
            $("#newTable .ligne_choice:last").append($(this));       
    });
...
//add choices
function addFormChoice(multipleResponse, uniqueResponse, add, responseNumber, response, point, comment, expectedResponse, order, sourceImageAdd, source_image_del, ChoicepositionForce) {

    $("#ujm_exobundle_interactionqcmtype_typeQCM")
        .find("option")
        .remove()
        .end()
        .append('<option selected="selected" value="1">'+multipleResponse+'</option><option value="2">'+uniqueResponse+'</option>')

    $("#ujm_exobundle_interactionqcmtype_choices").before('<a href="#" id="add_choice"><img src="' + sourceImageAdd + '">'+add+'</a>');

    $("#ujm_exobundle_interactionqcmtype_choices").after('<table style="border: 1px solid black;" id="newTable"><tr> <th>'+response+'</th> <th>'+point+'</th> <th>'+comment+'</th> <th>'+expectedResponse+'</th> <th>'+order+'</th> <th>'+ChoicepositionForce+'</th> <th>------</th> </tr></table>');
    $("#add_choice").css({
            "display": "block",
            "color": "green",
            "float": "right"
...
function addFormChoiceEdit(multipleResponse, uniqueResponse, add, responseNumber, response, point, comment, expectedResponse, order, sourceImageAdd, nbResponses, typeQCM) {
...
if (typeQCM == 1) {
    $("#ujm_exobundle_interactionqcmtype_typeQCM").append('<option selected="selected" value="1">'+multipleResponse+'</option><option value="2">'+uniqueResponse+'</option>')
}
else {
    $("#ujm_exobundle_interactionqcmtype_typeQCM").append('<option value="1">'+multipleResponse+'</option><option selected="selected" value="2">'+uniqueResponse+'</option>')
}
...
$("#ujm_exobundle_interactionqcmtype_choices").after('<table style="border: 1px solid black;" id="newTable"><tr> <th>'+responseNumber+'</th> <th>'+response+'</th> <th>'+point+'</th> <th>'+comment+'</th> <th>'+expectedResponse+'</th> <th>'+order+'</th> <th>------</th> <th>------</th> </tr></table>');
$("#add_choice").css({
        "display":"block",
        "color": "green",
        "float":"right"
