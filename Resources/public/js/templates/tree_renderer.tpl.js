    ...
    function($templateCache) {
        $templateCache.put('tree_renderer.tpl',
            '<div class="tree-node tree-node-content menu-item-container"  data-ng-class="{\'selected\':vm.menu.currentPageNode.id==node.id}" ng-click="vm.menu.changeCurrentPageNode(node)"></span>'+
                '<span data-ng-style="{\'margin-left\':((depth()-1)*15)+\'px\',}" class="menu-dropdown-icon"><a ng-if="node.children && node.children.length > 0" data-nodrag ng-click="toggle(this)"><span class="fa" ng-class="{\'fa-angle-right\': collapsed, \'fa-angle-down\': !collapsed}"></span></a></span>' +
