...
trait HasOrganizationsTrait
{
     ...
     * This will only return the list of organizations also owned by the current user.
     */
    #[Route(path: '/{id}/organization', name: 'list_organizations', methods: ['GET'], priority: 1)]
    public function listOrganizationsAction(string $id, #[CurrentUser] ?User $user): JsonResponse
    {
        $this->checkPermission('IS_AUTHENTICATED_FULLY', null, [], true);

        $object = $this->crud->get(static::getClass(), $id);
        if (empty($object)) {
            throw new NotFoundHttpException(sprintf('Object of class %s with id %s not found', static::getClass(), $id));
        }

        if (!method_exists($object, 'getOrganizations')) {
            throw new \LogicException(sprintf('Object of class %s does not have getOrganizations method', static::getClass()));
        }

        return new JsonResponse(array_map(function (Organization $organization) {
            return $this->serializer->serialize($organization, [SerializerInterface::SERIALIZE_LIST]);
        }, $this->getObjectOrganizations($object, $user)));
    }
    ...
    public function addOrganizationsAction(string $id, Request $request, #[CurrentUser] ?User $user): JsonResponse
    {
    ...
    public function removeOrganizationsAction(string $id, Request $request, #[CurrentUser] ?User $user): JsonResponse
    {
        // no need to secure entrypoint, the CRUD will do it for us.
    ...
    private function getObjectOrganizations(object $object, User $currentUser): array
    {
        $userOrganizations = array_map(function (Organization $organization) {
            return $organization->getUuid();
        }, $currentUser->getOrganizations());

        $objectOrganizations = is_array($object->getOrganizations()) ? $object->getOrganizations() : $object->getOrganizations()->toArray();

        return array_filter($objectOrganizations, static function (Organization $organization) use ($userOrganizations) {
            return in_array($organization->getUuid(), $userOrganizations);
        });
    }
}
