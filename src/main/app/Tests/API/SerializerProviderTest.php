...
class SerializerProviderTest extends TransactionalTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // this is hacky
        // I need to allow anonymous to see users email otherwise the serialized data cannot pass the validation
    ...
    {
        $samplesPath = $this->schema->getSampleDirectory($class).'/json/valid/create';
        if (!is_dir($samplesPath)) {
            return;
        }

        $iterator = new \DirectoryIterator($samplesPath);
    ...
    public function getHandledClassesProvider(): array
    {
        ...
        return array_filter($classes, function ($class) use ($schemaProvider) {
