...
const StatusChart = (props) => {
  const [data] = useFetch(props.name, props.url)
  ...
  return (
    <div className="card mb-4" role="presentation">
      <div className="card-body p-4 d-flex flex-row align-items-center justify-content-center gap-4">
        <ResponsiveContainer height={260} width={260} className="mb-n5">
          <PieChart>
            <Pie
        ...
        {data &&
          <ul className="list-unstyled mb- flex-fill">
            {data.map((d, i) => (
              <li key={d.status} className={classes('py-2 d-flex flex-row align-items-center gap-3 flex-wrap', {
                'border-top': i !== 0
              })}>
