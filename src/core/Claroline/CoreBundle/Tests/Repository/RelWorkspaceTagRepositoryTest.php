...
class RelWorkspaceTagRepositoryTest extends AltRepositoryTestCase
{
    private static $repo;
...
//        );
    ...
    public function testFindByWorkspaceAndUser()
    {
        ...
        $userTagRelations = self::$repo->findByWorkspaceAndUser($userWorkspace, $user);
        $adminTagRelations = self::$repo->findByWorkspaceAndUser($adminWorkspace, $admin);
    ...
    {
        $user = self::getUser('user');
        $tagRelations = self::$repo->findByUser($user);
        $this->assertEquals(4, count($tagRelations));
    }
