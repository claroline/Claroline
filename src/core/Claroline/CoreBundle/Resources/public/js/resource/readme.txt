...
Each resource must be rendered in a <span class='res-block'> This block must have the following data-attributes:
    data-id="{{ instance.id }}"
    data-type="{{ instance.type }}"
    data-resource-id="{{ instance.resource_id }}"
    Navigation:
        must be defined in this span
        is a link from the class "link-navigate-instance"
    ...
    Selection:
        ...
        <input class="chk-instance"type="checkbox" value="{{ instance.id }}">
    Thumbnails:
