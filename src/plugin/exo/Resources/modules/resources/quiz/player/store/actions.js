...
import {normalize, denormalizeAnswers, denormalize} from '#/plugin/exo/resources/quiz/player/normalizer'
...
actions.play = (previousPaper = null) => {
  return (dispatch, getState) => {
    ...
    if (!playerSelectors.offline(getState())) {
      ...
      // Promise is to expose the same interface as when there are async calls
...
actions.processEnd = (paper, navigate) => (dispatch, getState) => {
  ...
  dispatch(actions.handleAttemptEnd(newPaper, null, navigate))
}

actions.handleAttemptEnd = (paper, attempt, navigate) => {
  return (dispatch, getState) => {
    if (isEmpty(attempt)) {
      // test mode or anonymous
      // generate base evaluation attempt
      attempt = {
        startedAt: paper.startDate,
        endedAt: paper.endDate,
        lastActivityAt: paper.endDate,
        scoreMin: 0,
        score: paper.score,
        scoreMax: paper.total,
        rawScore: {
          current: paper.score,
          total: paper.total
        },
