...
class SessionManager
{
    ...
    public function generateFromTemplate(Session $session, string $locale): string
    {
        ...
        $placeholders = array_merge([
            ...
            'session_public_registration' => $this->translator->trans($session->getPublicRegistration() ? 'yes' : 'no', [], 'platform', $locale),
            'session_max_users' => $session->getMaxUsers(),
            'session_price' => $session->getPrice(),
            'session_price_description' => $session->getPriceDescription(),
            'location' => $location ? nl2br($location->getName().PHP_EOL.$location->getAddress()) : $this->translator->trans('online_session', [], 'cursus', $locale),
