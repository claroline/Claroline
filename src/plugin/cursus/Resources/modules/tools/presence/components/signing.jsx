        ...
        <div className="row py-4 bg-body-secondary rounded-2">
          <ContentHtml className="text-center mb-3">
            {trans('presence_info', {
              ...
              event_datetime_start: displayDate(props.currentEvent.start, true, true),
              event_datetime_end: displayDate(props.currentEvent.end, true, true)
            }, 'presence')}
          </ContentHtml>
          <ContentSizing size="sm" className="d-flex flex-column align-items-center mt-3">
            <input
              className="form-control"
              placeholder={trans('sign_label', {}, 'presence')}
              onChange={(event) => {props.setSignature(event.target.value.trim())}}
            />
            <Button
              className="btn btn-md btn-primary mt-3 mb-3 "
              type={CALLBACK_BUTTON}
              label={trans('sign', {}, 'presence')}
              primary={true}
              disabled={props.signature !== props.currentUser.name}
              callback={() => {props.signPresence(props.currentEvent)}}
            />
          </ContentSizing>
        </div>
      </Form>
...
const SignPresence = connect(
  ...
  (dispatch) => ({
    signPresence: (events, status) => {
      dispatch(actions.signPresence(events, status))
    },
    setSignature: (sign) => {
