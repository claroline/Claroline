...
import {getActions} from '#/plugin/agenda/events/task/utils'

const STORE_NAME = 'agendaTaskAbout'

const TaskAboutModal = (props) => {
  const dispatch = useDispatch()

  const contextPath = useSelector(contextSelectors.path)
  const currentUser = useSelector(securitySelectors.currentUser)
  const [event, status] = useFetch(STORE_NAME, ['apiv2_task_get', {id: get(props.event, 'id')}])
