...
const FORM_NAME = 'agendaTaskForm'
...
const TaskFormModal = props => {
  ...
  return (
    <FormModal
      title={trans(props.isNew ? 'new_task' : 'task', {}, 'agenda')}
      subtitle={props.isNew && props.session ? trans('new_event_desc', {}, 'agenda') : undefined}
      {...omit(props, 'event')}
      name={FORM_NAME}
      target={props.isNew ?
        ['apiv2_task_create'] :
        ['apiv2_task_update', {id: props.event.id}]
      }
      isNew={props.isNew}
      data={props.event}
      saveLabel={trans(props.isNew ? 'add_task' : 'save_task', {}, 'actions')}
      definition={[
        {
          title: trans('general'),
          primary: true,
